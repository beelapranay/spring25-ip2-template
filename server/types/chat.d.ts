 ...
 * participants is an array of usernames (strings) to match test expectations.
 ...
 * Uses userId to match test expectations.
 */
...
/**
 * Represents a Chat with participants and messages (fully enriched).
 * participants can be either string[] (usernames) or ObjectId[] depending on population state.
 * messages is an array of MessageInChat objects.
 */
export interface Chat {
  _id?: ObjectId;
...
/**
 * Payload for adding a participant to a chat.
 * Fixed to match test expectations - using userId instead of username.
 */
export interface AddParticipantPayload {
  userId: string;
}
